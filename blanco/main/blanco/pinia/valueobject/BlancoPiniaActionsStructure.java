...
/**
 * VueStore Actionsのフィールドをあらわすバリューオブジェクトクラス。このクラスの設定情報をもとにフィールドとセッター・ゲッターが自動生成されます。
 */
public class BlancoPiniaActionsStructure {
    /**
     * 項目番号。省略可能です。
     *
     * フィールド: [no]。
    ...
    /**
     * フィールド [async] の値を設定します。
     *
     * フィールドの説明: [非同期かどうか]。
     *
     * @param argAsync フィールド[async]に設定する値。
     */
    public void setAsync(final Boolean argAsync) {
        fAsync = argAsync;
    }

    /**
     * フィールド [async] の値を取得します。
     *
     * フィールドの説明: [非同期かどうか]。
     * デフォルト: [false]。
     *
     * @return フィールド[async]から取得した値。
     */
    ...
    public String toString() {
        ...
        buf.append("blanco.pinia.valueobject.BlancoPiniaActionsStructure[");
        buf.append("no=" + fNo);
     ...
     */
    public void copyTo(final BlancoPiniaActionsStructure target) {
        if (target == null) {
            throw new IllegalArgumentException("Bug: BlancoPiniaActionsStructure#copyTo(target): argument 'target' is null");
        }
